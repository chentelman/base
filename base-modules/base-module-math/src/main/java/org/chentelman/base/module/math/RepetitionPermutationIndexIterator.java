...
/**
 * This class implements a simple iterator over the permutations for n selections with repetition
 * The values for the combination is in the range [0,n) to allow them to be
 * used as indexes to an other set of item
 */
public class RepetitionPermutationIndexIterator implements Iterator<int[]> {
	protected int[]   state;
	 ...
	 */
	public RepetitionPermutationIndexIterator (int k, int n) {
		this.count = n;
		this.state = RepetitionPermutations.init(k, n);
